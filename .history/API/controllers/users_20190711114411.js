...
exports.LOGIN_USER = (req, res) => {
    ...
    User.findOne({ email }).exec()
        ...
        .catch(err => {
           return res.status(500).json({ success: false, Error: 'Internal server error during sign in uesr: ', err })
        })
}

exports.ADD_USER_COOKIE = (req, res, next) => {
    const user = req.body.user
    res.cookie('User', user ).status(200).json({ message: 'Cookie set successfully :', user  })
}

exports.GET_USER_COOKIE = (req, res, next) => {
        return res.status(200).json({ success: true, message: user, cookies: res.cookie})
}
